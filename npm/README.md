# not-ace-tool-rs npm shim (Legacy)

> **Vendored Not ACE client:** This package metadata is for the Not ACE fork vendored in the ACE workspace. Release URLs, cache names, and binaries use `not-ace-tool-rs`; historical upstream links are explicitly labeled.
...
This npm package provides a convenient way to install and run `not-ace-tool-rs` via npm/npx.
...
| Platform | Cache Path |
|----------|------------|
| Windows  | `%LOCALAPPDATA%\not-ace-tool-rs\<version>\` |
| macOS    | `~/Library/Caches/not-ace-tool-rs/<version>/` |
| Linux    | `$XDG_CACHE_HOME/not-ace-tool-rs/<version>/` or `~/.cache/not-ace-tool-rs/<version>/` |
...
1. **Manual download**: Download the appropriate binary from the Not ACE GitHub Releases page and place it in the cache directory.
