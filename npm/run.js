...
function getAssetName() {
  ...
  switch (platform) {
    case "darwin":
      ...
      return "not-ace-tool-rs_Darwin_universal.tar.gz";
    case "linux":
      if (arch === "x64") {
        return "not-ace-tool-rs_Linux_x86_64.tar.gz";
      } else if (arch === "arm64") {
        return "not-ace-tool-rs_Linux_aarch64.tar.gz";
      }
      throw new Error(
    ...
    case "win32":
      if (arch === "x64") {
        return "not-ace-tool-rs_Windows_x86_64.zip";
      } else if (arch === "arm64") {
        return "not-ace-tool-rs_Windows_aarch64.zip";
      }
      throw new Error(
...
async function downloadAndExtract(cacheDir) {
  ...
  try {
    ...
    const version = getPackageVersion();
    console.error(`Downloading ${DISPLAY_PACKAGE_NAME} v${version}...`);
    ...
    console.error(`Installed ${DISPLAY_PACKAGE_NAME} to ${binaryPath}`);
    return binaryPath;
  } catch (error) {
    console.error(`Failed to download ${DISPLAY_PACKAGE_NAME}: ${error.message}`);
    console.error("");
    ...
    console.error("  cargo install not-ace-tool-rs");
    process.exit(1);
  } finally {
