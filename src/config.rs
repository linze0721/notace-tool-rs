...
pub struct ConfigOptions {
    ...
    pub enable_workflow_tools: Option<bool>,
...
impl Config {
    ...
    pub fn new(base_url: String, token: String, options: ConfigOptions) -> Result<Arc<Self>> {
        // Ensure remote base URLs use https:// while preserving explicit localhost HTTP
        // endpoints for local development and smoke tests.
        let base_url = if let Some(rest) = base_url.strip_prefix("http://") {
            if is_local_http_host(rest) {
                base_url
            } else {
                format!("https://{}", rest)
            }
        ...
        Ok(Arc::new(Self {
            base_url,
            token,
            container_tag: options
                .container_tag
                .or_else(|| std::env::var("ACE_CONTAINER_TAG").ok())
                .unwrap_or_else(|| "default".to_string()),
            enable_memory_tools: options
                .enable_memory_tools
                .unwrap_or_else(|| env_flag_enabled("ACE_ENABLE_MEMORY_TOOLS", true)),
            enable_taste_tools: options
                .enable_taste_tools
                .unwrap_or_else(|| env_flag_enabled("ACE_ENABLE_TASTE_TOOLS", true)),
            enable_goal_tools: options
                .enable_goal_tools
                .unwrap_or_else(|| env_flag_enabled("ACE_ENABLE_GOAL_TOOLS", true)),
            enable_workflow_tools: options
                .enable_workflow_tools
                .unwrap_or_else(|| env_flag_enabled("ACE_ENABLE_WORKFLOW_TOOLS", true)),
    ...
    pub fn new_for_third_party_enhancer() -> Arc<Self> {
        Arc::new(Self {
            base_url: String::new(),
            token: String::new(),
            container_tag: std::env::var("ACE_CONTAINER_TAG")
            ...
            enable_goal_tools: env_flag_enabled("ACE_ENABLE_GOAL_TOOLS", true),
            enable_workflow_tools: env_flag_enabled("ACE_ENABLE_WORKFLOW_TOOLS", true),
...
fn is_local_http_host(rest: &str) -> bool {
    let authority = rest
        .split('/')
        .next()
        .unwrap_or_default()
