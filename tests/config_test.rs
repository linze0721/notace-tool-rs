...
fn clear_not_ace_env() -> [EnvVarGuard; 5] {
    [
        ...
        EnvVarGuard::clear("ACE_ENABLE_WORKFLOW_TOOLS"),
    ]
}
...
fn test_config_default_values() {
    ...
    assert!(config.enable_workflow_tools);
...
fn test_config_with_custom_values() {
    let config = Config::new(
        ...
        ConfigOptions {
            ...
            enable_workflow_tools: Some(false),
            no_adaptive: true,
...
fn test_config_options_default() {
    ...
    assert!(options.enable_workflow_tools.is_none());
...
fn test_config_reads_container_and_tool_flags_from_env() {
    ...
    let _workflow = EnvVarGuard::set("ACE_ENABLE_WORKFLOW_TOOLS", "false");
...
fn test_config_env_flags_disabled_values() {
    ...
    let _workflow = EnvVarGuard::clear("ACE_ENABLE_WORKFLOW_TOOLS");
    ...
    for disabled_value in ["disabled", "false", "off", "0", " Disabled "] {
        ...
        std::env::set_var("ACE_ENABLE_WORKFLOW_TOOLS", disabled_value);
        let config = test_config("https://api.example.com", "test-token").unwrap();
        ...
        assert!(
            !config.enable_workflow_tools,
            "workflow flag {disabled_value}"
        );
    }
    ...
    std::env::set_var("ACE_ENABLE_WORKFLOW_TOOLS", "true");
    let config = test_config("https://api.example.com", "test-token").unwrap();
    assert!(config.enable_memory_tools);
    assert!(config.enable_taste_tools);
    assert!(config.enable_goal_tools);
