...
为 AI 编程助手提供代码库搜索、持久记忆、偏好学习和任务规划能力的 MCP 服务器。
...
3. 完成：重启你的 Agent，然后使用 `search_context`、`recall` 和 `plan`。
...
not-ace-tool-rs 通过 MCP 把 AI 编程助手连接到你的代码库。它会索引文本文件，用自然语言查询找出相关代码，也能结合项目上下文增强提示词。它还可以保存长期记忆、召回历史决策、学习用户偏好，并在多次会话之间管理任务计划。
