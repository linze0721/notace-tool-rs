...
not-ace-tool-rs 是面向 AI 编程助手的高性能 MCP 服务器。它通过 Model Context Protocol 提供代码搜索、AI 增强目标规划、智能工作流工具、持久化记忆/学习、偏好学习和 Web 研究能力。
...
执行后：`npx` 会下载匹配当前平台的软件包，启动 `not-ace-tool-rs`，连接你的 API，并通过 stdio 提供 MCP 工具。日常使用建议加入 MCP 客户端配置，让助手自动启动它。

支持 Linux (x64/ARM64)、macOS (x64/ARM64)、Windows (x64/ARM64)。

## v0.6.0 新增内容

- 新增 `diagnose`：结合记忆、代码搜索和 Web 研究诊断错误，并将解决方案写回记忆。
- 新增 `code_review`：基于代码库上下文、项目记忆和偏好档案审查 diff。
- 新增 `generate_docs`：基于代码分析生成项目、模块或文件级文档。
- 改进复杂 MCP 参数的 Serde 解析：数组和对象既可以是原生 JSON，也可以是字符串化 JSON。
...
| **文档生成** | `generate_docs` | 基于代码分析生成项目/模块/文件文档 |
...
本节按使用场景组织工具。每组先说明为什么使用它，再列出精确的工具名和参数名，方便直接发起 MCP 调用。
...
基于代码搜索和项目记忆，生成项目、模块或文件级别的文档。支持不同范围，自动将生成的文档存入记忆。

| 参数 | 类型 | 必填 | 说明 |
|---|---|---|---|
| `project_root_path` | string | 是 | 项目根路径 |
| `scope` | string | 否 | `overview`（默认）、`module:路径` 或 `file:路径` |
